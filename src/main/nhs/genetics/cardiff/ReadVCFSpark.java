...
public class ReadVCFSpark {
    ...
    private ArrayList<HashMap<VariantContextWrapper, ArrayList<FrameworkSparkFilter.Workflow>>> stratifiedVariants = new ArrayList<>();
    private SparkConf sparkConf;
    private JavaSparkContext javaSparkContext;
    private JavaRDD<VariantContext> informativeVariants;

    public ReadVCFSpark(File vcf, boolean onlyPrintKnownRefSeq, Integer threads){
        this.vcf = vcf;
        this.onlyPrintKnownRefSeq = onlyPrintKnownRefSeq;

        //initalise spark
        this.sparkConf = new SparkConf().setAppName(Main.PROGRAM).setMaster("local[" + threads + "]");
    ...
    public void gatherCohortInformativeVariants(){
        this.informativeVariants = javaSparkContext.textFile(vcf.toString())
    ...
    public ArrayList<HashMap<VariantContextWrapper, ArrayList<FrameworkSparkFilter.Workflow>>> stratifyCandidateVariants() {
        ...
        for (int n = 0; n < samples.size(); ++n){
            ...
            if (sample.getAffection() == Affection.AFFECTED){
                ...
                JavaRDD<VariantContext> informativeGenotypes = informativeVariants.filter(new NonVariantBySampleSparkFilter(sample.getID()));
